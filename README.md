...
### 1단계 — 개발 환경 자동 설정 (최초 1회)

클론 직후 아래 명령 하나로 필요한 모든 도구를 설치합니다.

```bash
git clone https://github.com/your-org/finance-city-engine.git
cd finance-city-engine
...
| 항목 | Linux (Ubuntu) | macOS |
| ------ | ------ | ------ |
| 빌드 도구 | cmake, ninja-build, clang-18 | cmake, ninja, llvm (brew) |
| Vulkan SDK | LunarG apt 또는 직접 다운로드 | brew (vulkan-loader, molten-vk) |
| vcpkg | `~/vcpkg` 자동 클론 + bootstrap | 동일 |
| 환경 변수 | `~/.bashrc` / `~/.zshrc` 자동 기록 | `~/.zshrc` 자동 기록 |

설치 완료 후 **셸 재시작 또는 환경 변수 로드**:
...
### 2단계 — 데스크탑 빌드 및 실행 (Vulkan)

```bash
make build
./build/FinanceCityEngine
```

> vcpkg 의존성(`glfw3`, `glm`, `imgui` 등)은 `make build` 시 **자동 설치**됩니다.

---

### 3단계 — 웹 빌드 (WebGPU + WASM, 선택)

```bash
# Emscripten SDK 설치 (최초 1회, ~5분)
