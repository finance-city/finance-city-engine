...
/**
 * @brief ImGui UI manager
 *
 * Responsibilities:
 * - ImGui backend management (Vulkan/WebGPU via adapter pattern)
 * - UI rendering (camera controls, lighting settings, particle triggers)
 */
class ImGuiManager {
public:
    ...
    // Lighting settings (set by UI, read by Application)
    struct LightingSettings {
        glm::vec3 sunDirection{0.7f, 0.25f, 0.5f};
        float sunIntensity = 1.2f;
        glm::vec3 sunColor{1.0f, 0.6f, 0.3f};
